...
针对 trainer.py（官方模型 + 训练好的 projection 层）
"""
...
MODEL_PATH = os.path.join(ASTROCLIP_ROOT, "outputs123/trained_models/astroclip_best_model.pt")
...
def evaluate_cross_modal_retrieval(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Feature Extraction"):
            ...
            # AstroClipModel.forward() 返回 (img_feat, spec_feat)
...
def main():
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            f"找不到权重文件 {MODEL_PATH}，请确认 trainer.py 是否已经训练完毕！"
        )
