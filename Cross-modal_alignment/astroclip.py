...
class CLIPLoss(nn.Module):
    """原生 CLIP 对比损失"""
    def get_logits(self, image_features: torch.Tensor, spectrum_features: torch.Tensor, logit_scale: float) -> Tuple[torch.Tensor, torch.Tensor]:
        image_features = F.normalize(image_features, dim=-1, eps=1e-3)
        spectrum_features = F.normalize(spectrum_features, dim=-1, eps=1e-3)
        logits_per_image = logit_scale * image_features @ spectrum_features.T
        return logits_per_image, logits_per_image.T

    def forward(self, image_features: torch.Tensor, spectrum_features: torch.Tensor, logit_scale: float = 15.5):
        ...
        loss = (F.cross_entropy(logits_per_image, labels) + F.cross_entropy(logits_per_spectrum, labels)) / 2
        return loss

# class AstroClipModel(nn.Module): 
#     """纯 PyTorch AstroCLIP 模型""" 
#     def __init__(self, image_encoder: nn.Module, spectrum_encoder: nn.Module, logit_scale: float = 15.5): 
#         super().__init__()
#         self.image_encoder = image_encoder 
#         self.spectrum_encoder = spectrum_encoder 
...
#         loss = self.criterion(img_feat, spec_feat, self.logit_scale)
#         return loss

class AstroClipModel(nn.Module):
    """纯 PyTorch AstroCLIP 模型"""
    #trainer.py 无attention使用
    # def __init__(self, image_encoder: nn.Module, spectrum_encoder: nn.Module):
    #     super().__init__()
    #     self.image_encoder = image_encoder
    #     self.spectrum_encoder = spectrum_encoder

    #     # ✅ 改1：learnable logit_scale（CLIP标准）
    ...
    def __init__(self, image_encoder: nn.Module, spectrum_encoder: nn.Module):
        ...
        # ✅ 新增：统一 embedding 维度
        self.img_proj = nn.Linear(1024, 1024)
        self.spec_proj = nn.Linear(768, 1024)
    ...
    def forward(self, images: torch.Tensor, spectrum: torch.Tensor):
        ...
        spec_feat = self.spectrum_encoder(spectrum)  # (B, 768)
    ...
    def compute_loss(self, images: torch.Tensor, spectrum: torch.Tensor):
        ...
        loss = self.criterion(img_feat, spec_feat, logit_scale)
        return loss
